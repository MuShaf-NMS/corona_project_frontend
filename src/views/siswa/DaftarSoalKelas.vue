          ...
          <b-card-text>
            <b-row v-for="(i,index) in items" :key="index" class="card-dest">
              <b-col v-for="(j,index) in i" :key="index" md="4">
                <soal-kelas :uuid="j.uuid_mapel" :mapel="j.mapel" />
              </b-col>
            </b-row>
          </b-card-text>
        </b-card>
...
<script>
import SoalKelas from "../../components/siswa/SoalKelas";
...
export default {
  ...
  methods: {
    async loadData() {
      try {
        let data = await siswa.getSoalKelas();
        this.items = this.triple(data.data.mapel);
        this.title = `Daftar Soal Kelas ${data.data.kelas.kelas}`;
      } catch (err) {
        logout.clear();
      }
    },
