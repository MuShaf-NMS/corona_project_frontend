          ...
          <b-card-text>
            <b-table striped hover :items="siswa" :fields="fields">
              ...
              <template v-slot:cell(hapus)="row">
                <b-btn size="sm" @click="hapus(row.item.uuid,row.item.nama)"><b-icon icon="trash"></b-icon></b-btn>
              </template>
            </b-table>
          </b-card-text>
        </b-card>
...
export default {
  ...
  data() {
    return {
      siswa: [],
      fields: ["nama", {key: "jk", label: "Jenis kelamin"}, {key: "created_at", label: "Dibuat"}, {key: "updated_at", label: "Update terakhir"}, "update", "hapus"],
  ...
  methods: {
    async loadData() {
      try {
        let data = await user.getSiswa(this.$route.params.kelas,this.$route.params.cabang);
